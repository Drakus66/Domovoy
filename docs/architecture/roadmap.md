...
> на предложения — уже в 2C (`/proposals`). **Дальше:** drag-connect-save редактор (нужен `PUT /api/blocks`),
> сохранение раскладки, мульти-триггер в редакторе правил.
...
- **2M.2 — Notification/comms-порт + stub.** Абстракция, в которую плагин публикует уведомления; disabled-stub
  сейчас (+ симуляция в эмуляторе), реальный канал (Telegram и т.п.) — позже. Гасит абстракцией долг 2G.
...
### Эпик 2N. «Дневник дома»: художественная летопись жизни дома 🔵 (концепт, план зафиксирован 2026-07-06)
