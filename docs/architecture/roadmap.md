...
- **Модель правила** `trigger → condition → action` поверх существующей [Automation](../../src/Gateway/Domovoy.DbGateway/Models/Automation.cs):
...
- **DoD:** правило «движение в коридоре после заката → включить свет на 5 мин» создаётся, сохраняется, срабатывает и пишется в историю; защитное правило работает даже при недоступном UI.
...
- **DoD:** каждая capability имеет адаптер и сквозной путь команда→исполнение→подтверждение состояния; домены управляются правилами из 1A.
...
| **1. Автоматизация** | ближайшее | дом работает по сценариям; копятся данные; действия объяснимы | AutomationService, TSDB, Integration SDK, climate/heating/irrigation адаптеры, объяснимость+реплей (1F), режимы дома (1G) |
