...
> 1B ✅ (платформа данных телеметрии) · 1F ✅ (объяснимость + реплей) · 1H ✅ (control blocks — E1-фундамент) ·
> 1D ✅ (доменные контуры + актуация) · 1C/1E ⬜. Рантайм против реального RabbitMQ/Mongo не прогонялся.
...
| **1. Автоматизация** | сейчас | дом работает по сценариям; копятся данные; действия объяснимы | AutomationService (1A ✅), режимы дома+присутствие (1G ✅), платформа данных телеметрии (1B ✅), объяснимость+реплей (1F ✅), control blocks (1H ✅ E1), climate/heating/irrigation контуры (1D ✅), Integration SDK (1C), визуальный редактор (1E) |
