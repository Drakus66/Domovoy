...
> Фаза 1 функционально закрыта; план Фазы 2 согласован (см. ниже). Закрытие P0-4 + offline-smoke вынесено в Фазу 1.5.
...
> **Пререквизит — Фаза 1.5 (верификация рантайма, S–M).** Закрыть интеграционный тест сквозного пути (P0-4) +
> **offline-smoke** (ядро работает при отключённой внешней сети) + живой 3-оконный прогон по
> [`runbook.md`](../runbook.md) против реального RabbitMQ/Mongo. Весь стек собран/типизирован/юнит-тестирован,
> но **вживую не гонялся** — Фаза 2 строится поверх. **DoD:** каждый ключевой поток зелёный против реальной
> инфры; offline-smoke в CI.
...
### Эпик 2G. Центр активности (события + логи + уведомления) ⬜
...
| **1.5 Верификация** | пререквизит | стек проверен вживую | интеграционный тест P0-4 + offline-smoke + 3-оконный прогон против реального RabbitMQ/Mongo |
| **2. Интеллект** | среднесрочно | дом подсказывает; умнее распознаёт устройства; читаемый центр активности | ML-блоки на ML.NET (2A/2B, shadow→bounded→full), очередь предложений (2C), типизация устройств (2D), роли (2E), центр активности (2G), LLM-заглушки (2H) |
