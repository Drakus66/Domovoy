...
| **2. Интеллект** | 🚧 начата | дом подсказывает; умнее распознаёт устройства; читаемый центр активности | типизация устройств (2D ✅), центр активности (2G ✅), ML-блоки на ML.NET (2A/2B, shadow→bounded→full), очередь предложений (2C), роли (2E), LLM-заглушки (2H) |
