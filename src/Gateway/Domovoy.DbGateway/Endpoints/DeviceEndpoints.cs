...
public static class DeviceEndpoints
{
    public static void MapDeviceEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        // Get all devices
        group.MapGet("/", async (IDeviceRepository repo) =>
        {
        ...
        // Get device by ID
        group.MapGet("/{id}", async Task<Results<Ok<DeviceDto>, NotFound>> (string id, IDeviceRepository repo) =>
        {
        ...
        group.MapGet("/location/{location}", async (string location, IDeviceRepository repo) =>
        {
        ...
        // Get devices by type
        group.MapGet("/type/{type}", async (string type, IDeviceRepository repo) =>
        {
        ...
        // Create device
        group.MapPost("/", async Task<Results<Created<DeviceDto>, BadRequest>> (CreateDeviceDto deviceDto, IDeviceRepository repo) =>
        {
        ...
        // Update device
        group.MapPut("/{id}", async Task<Results<Ok<DeviceDto>, NotFound>> (string id, UpdateDeviceDto deviceDto, IDeviceRepository repo) =>
        {
        ...
        group.MapPatch("/{id}/state",
            async Task<Results<Ok<DeviceDto>, NotFound>> (string id, UpdateDeviceStateDto stateDto, IDeviceRepository repo) =>
            {
                var existingDevice = await repo.GetByIdAsync(id);
                if (existingDevice == null) return TypedResults.NotFound();
        ...
        group.MapPatch("/{id}/online", async Task<Results<Ok<DeviceDto>, NotFound>> (string id, bool isOnline, IDeviceRepository repo) =>
        {
            var existingDevice = await repo.GetByIdAsync(id);
            if (existingDevice == null) return TypedResults.NotFound();
        ...
        // Delete device
        group.MapDelete("/{id}", async Task<Results<Ok, NotFound>> (string id, IDeviceRepository repo) =>
        {
            var device = await repo.GetByIdAsync(id);
            if (device == null) return TypedResults.NotFound();
