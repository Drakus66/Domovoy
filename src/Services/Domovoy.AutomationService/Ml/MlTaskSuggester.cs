...
public sealed class MlTaskSuggester : BackgroundService
{
    ...
    public MlTaskSuggester(
        DbGatewayClient db, MlTrainingService training, BlockCatalog catalog,
    ...
    public async Task<ScanResult> ScanOnceAsync(CancellationToken ct)
    {
        ...
        if (tasks is null) return new ScanResult(0, 0, "ml_tasks unavailable");
        ...
        var taken = new HashSet<string>(
            tasks.Select(t => t.TargetCapability)
                .Concat(open.Where(p => p.Kind == ProposalKind.MlTask && p.MlTaskTarget is not null).Select(p => p.MlTaskTarget!)),
